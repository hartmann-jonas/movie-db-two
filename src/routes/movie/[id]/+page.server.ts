...
export const actions: Actions = {
	...
	likeMovie: async ({ locals, params }) => {
		...
		if(locals.user.id) {
			...
			const response = await fetch(`https://api.themoviedb.org/3/movie/${params.id}/keywords?api_key=${process.env.TMDB_API_KEY}`)
            if(response.ok) {
                keywords = await response.json()
            }
            console.log('Movie ID:  '+ movieId)
			console.log('User:  ' + userId)
            // link all the keywords to the movie
            for await (const keyword of keywords.keywords) {
